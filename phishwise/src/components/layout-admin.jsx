...
import { Icon, Button, Avatar, Chip } from "./ui.jsx";
import { useApp } from "../lib/store.jsx";
import { NOTIFICATIONS, ADMIN_NOTIFICATIONS } from "../data/mock.js";
...
const ADMIN_NAV = [
  ...
  { to: "/admin/analytics", label: "Platform Reports", icon: "PieChart" },
];
  ...
  const sidebar = (mobile = false) => (
    <div className={`flex h-full flex-col ${isAdminArea ? "bg-[#150f2e]" : "bg-ink-950"} text-white`}>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3" aria-label="Main navigation">
        ...
        {isAdmin && (
          <>
            {(!collapsed || mobile) && <p className="px-3 pb-1 pt-4 text-[11px] font-semibold uppercase tracking-wider text-ink-500">Administration</p>}
            {(collapsed && !mobile) && <div className="my-3 border-t border-white/10" />}
    ...
    <footer className="border-t border-ink-100 bg-white dark:border-ink-800 dark:bg-ink-950">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-4">
        <div className="md:col-span-2">
          <Logo />
          <p className="mt-3 max-w-sm text-sm text-ink-500 dark:text-ink-400">Gamified micro-learning for phishing awareness and cyber hygiene. A final-year cybersecurity project demo — built with React, no backend required.</p>
        </div>
        <div>
